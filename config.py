...
PIN_SERVO_TILT = (1, 29)         # 物理引脚26 = GPIO1_D5（头俯仰舵机）
...
SERVO_CENTER_ANGLE = 90          # 中心角度（正对前方，pan用）
SERVO_MIN_PULSE_US = 500         # 0° 对应脉宽 (μs)
SERVO_MAX_PULSE_US = 2500        # 180° 对应脉宽 (μs)
SERVO_SPEED_DEG_PER_S = 300      # 舵机转动速度 (°/s)，SG90 约 0.12s/60°

# 俯仰舵机角度映射（SG90 0°~180° → 屏幕仰角 5°~55°）
# 屏幕基础仰角 28°（机械），舵机推动额外 0°~27° 仰角调整
TILT_NEUTRAL = 28                # 默认仰角 (°)，平视中等身高
TILT_MIN = 5                     # 最低仰角（低头看近处/小孩）
TILT_MAX = 55                    # 最大仰角（抬头看高处/高个子）
TILT_SERVO_NEUTRAL = 90          # 舵机中位对应屏幕 28° 仰角
TILT_SERVO_RANGE = 50            # 舵机 ±50° 映射仰角 ±25°


# ============================================================
# 视觉追踪参数（双轴云台：pan + tilt）
# ============================================================

TRACKER_ANGLE_P_GAIN = 0.08      # 水平偏差 → 舵机角度的 P 增益
TRACKER_TILT_P_GAIN = 0.06       # 垂直偏差 → 俯仰舵机的 P 增益
...
TRACKER_TILT_LOST_ANGLE = 5      # 丢失目标后俯仰角（低头，看起来"失落"）
TRACKER_TILT_SMOOTH = 0.15       # 俯仰平滑系数（比pan更慢，避免频繁点头）
